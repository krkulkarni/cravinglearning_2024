...
def fit(params, craving_ratings, actions, outcomes, evs, rpes, prior=None, output='npl'):
    ...
    gamma = 0.97 # hard coded from median of fitted gammas
    craving_baseline = params[0]
    ev_weight = params[1]
    rpe_weight = params[2]
    ...
    for b in range(nblocks): #if nblocks==1, use reversals
        for t in range(ntrials):
            if craving_ratings[b, t] > -1:
                ...
                pred_cravings[b, t] = craving_baseline + ev_term + rpe_term
    ...
    if output == 'npl':
        ...
        subj_dict = {'params'     : [craving_baseline, ev_weight, rpe_weight],
                     'ev'         : evs, 
                     'actions'    : actions,
                     'outcomes'   : outcomes, 
                     'rpes'       : rpes, 
...
# class EVRPEGeoDecay(CravingPrototype):

#     def __init__(self, decision_model):

#         self.craving_name = 'EV RPE Geo Decay'
#         self.short_name = 'evrpe_geodecay'
#         self.parnames = ['craving_baseline', 'ev_weight', 'rpe_weight', 'gamma', 'craving_sd']
#         self.partransform = ['1*', '1*', '1*', 'expit', 'np.exp']
#         self.npars = len(self.parnames)

#         super().__init__(decision_model)

#     def llfunc(self, params, *args):
#         craving_baseline, ev_weight, rpe_weight, gamma, craving_sd = params
...
#                 [craving_baseline, ev_weight, rpe_weight, gamma, craving_sd], 
#                 map_prior[:, 0], map_prior[:, 1]
#             )
#         else:
#             prior_loglik = 0

#         if np.abs(craving_baseline) > 100 or np.abs(ev_weight) > 10 or np.abs(rpe_weight) > 10 or np.abs(craving_sd) > 10:
#             return 10000000
...
#                 pred_cravings.append(craving_baseline + ev_term + rpe_term)
