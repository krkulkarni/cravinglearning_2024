...
def fit(params, craving_ratings, actions, outcomes, evs, rpes, prior=None, output='npl'):
        ...
        for t in range(ntrials):
            if craving_ratings[b, t] > -1:
                ...
                for j in range(0, t+1):
                    outcome_term += outcomes[b, j] * gamma**(t-j)
                outcome_term *= outcome_weight
                ...
                pred_cravings[b, t] = craving_baseline + outcome_term + ev_term + rpe_term
    ...
    if output == 'npl':
        ...
        subj_dict = {'params'     : [gamma, craving_baseline, outcome_weight, ev_weight, rpe_weight],
                     'ev'         : evs, 
                     'actions'    : actions,
                     'outcomes'   : outcomes, 
                     'rpes'       : rpes, 
...
# class OutcomeEVRPEGeoDecay(CravingPrototype):

#     def __init__(self, decision_model):

#         self.craving_name = 'Outcome EV RPE Geo Decay'
#         self.short_name = 'oevrpe_geodecay'
#         self.parnames = ['craving_baseline', 'outcome_weight', 'ev_weight', 'rpe_weight', 'gamma', 'craving_sd']
#         self.partransform = ['1*', '1*', '1*', '1*', 'expit', 'np.exp']
#         self.npars = len(self.parnames)

#         super().__init__(decision_model)

#     def llfunc(self, params, *args):
#         craving_baseline, outcome_weight, ev_weight, rpe_weight, gamma, craving_sd = params
...
#                 [craving_baseline, outcome_weight, ev_weight, rpe_weight, gamma, craving_sd], 
#                 map_prior[:, 0], map_prior[:, 1]
#             )
#         else:
#             prior_loglik = 0

#         if np.abs(craving_baseline) > 100 or np.abs(outcome_weight) > 10 or np.abs(ev_weight) > 10 or np.abs(rpe_weight) > 10 or np.abs(craving_sd) > 10:
#             return 10000000
...
#                     outcome_term += rewards[j] * gamma**(i-j)
...
#                 pred_cravings.append(craving_baseline + outcome_term + ev_term + rpe_term)
