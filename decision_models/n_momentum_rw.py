...
def fit(params, choices, rewards, craving_ratings, prior=None, output='npl'):
    ...
    # Normalize craving ratings that are not NaN to be between -1 and 1
    craving_range = np.nanmax(craving_ratings) - np.nanmin(craving_ratings)
    craving_ratings = (2*(craving_ratings - np.nanmin(craving_ratings))/craving_range) - 1
    ...
    for b in range(nblocks): #if nblocks==1, use reversals
        for t in range(ntrials):
            ...
            # if any elements of ev are < 1e-3, return 10000000
            # if any(ev[b, t, :] < 1e-1):
            #     return 10000000
            ...
            # calculate PE
            biased_reward = rewards[b, t] * np.power(mod, m)
            pe[b, t] = biased_reward - ev[b, t, c]
            h += mom_lr * (pe[b, t] - h)
            m = np.tanh(h)
            # print(f'Mod: {mod}')
            # print(f'Craving: {craving_ratings[t]}')
            # print(f'Reward: {rewards[b, t]}')
            # print(f'Biased Reward: {biased_reward}')
            # print(f'Equation: {rewards[b, t]} * {mod}^{craving_ratings[t]}')
    ...
    if output == 'npl':
        ...
        subj_dict = {'params'     : [beta, lr, mod, mom_lr],
                     'ev'         : ev, 
                     'ch_prob'    : ch_prob, 
                     'choices'    : choices, 
                     'choices_A'  : choices_A, 
...
def simulate(params, block_type, raw_craving_ratings, nblocks=1, ntrials=60):
    ...
    for subj_idx in range(nsubjects):
        beta, lr, mod, mom_lr = params[subj_idx]
        beta = norm2beta(beta)
        lr   = norm2alpha(lr)
        mod  = np.exp(mod)
        mom_lr = norm2alpha(mom_lr)
        h = 0
        m = 0
        ...
        for b in range(nblocks): # if nblocks == 1, then use reversals
            ...
            for t in range(ntrials):
                ...
                # update values
                biased_reward = rewards[subj_idx, b, t] * np.power(mod, m)
                pe[subj_idx, b, t] = biased_reward - ev[subj_idx, b, t, c]
                h+= mom_lr * (pe[subj_idx, b, t] - h)
                m = np.tanh(h)

                ev[subj_idx, b, t+1, :] = ev[subj_idx, b, t, :].copy()
                ev[subj_idx, b, t+1, c] += lr * pe[subj_idx, b, t]
