...
def fit(params, choices, rewards, craving_ratings, prior=None, output='npl'):
    ...
    for b in range(nblocks): #if nblocks==1, use reversals
        for t in range(ntrials):
            ...
            biased_beta = norm2beta(beta + mod*craving_ratings[t])
            if biased_beta < min(this_beta_bounds) or biased_beta > max(this_beta_bounds):
                # print(f'beta = {beta:.3f} not in range')
                return 10000000
            ch_prob[b, t,:] = softmax(ev[b, t, :], biased_beta)
    ...
    for subj_idx in range(nsubjects):
        ...
        for b in range(nblocks): # if nblocks == 1, then use reversals
            ...
            for t in range(ntrials):
                ...
                # calculate choice probability
                biased_beta = norm2beta(beta + mod*craving_ratings[subj_idx, b, t])
                ch_prob[subj_idx, b, t,:] = softmax(ev[subj_idx, b, t, :], biased_beta)
